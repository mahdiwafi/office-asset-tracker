...
Captured from the live site (sign-in is Entra-gated, so they are taken by hand). Each is roughly 1200px wide.

| Asset catalog | Approval queue |
| --- | --- |
| ![Asset catalog with status badges](docs/screenshots/catalog.png) | ![Approval queue with pending requests and returns](docs/screenshots/approvals.png) |

| My loans | Raise a request |
| --- | --- |
| ![My loans with return-requested badge](docs/screenshots/loans.png) | ![Raise request form with date range](docs/screenshots/request.png) |

| Audit trail | Ask ICT |
| --- | --- |
| ![Audit log with before/after changes](docs/screenshots/audit.png) | ![Ask ICT answer with cited sources](docs/screenshots/ask-ict.png) |

## Testing approach

The suite is hermetic: **158 tests** run in CI against a real Postgres 16 (service container) with every external provider faked at our boundary — JWT validation against locally minted tokens and a mocked JWKS, the search client and the LLM HTTP call replaced by fakes that pin the wire contract (a provider mismatch fails the test, not a live 500). The race-condition and transaction-boundary tests are the ones the plan refused to skip: they prove the database constraints, not just the happy path.
