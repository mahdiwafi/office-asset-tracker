...
| 1.0 | ✅ Ten-pair golden set (`docs/golden-set.md` + `scripts/golden_set.py`) — 7/7 answerable queries retrieved their expected article in a live run (2026-08-24), the four refusal rows exercised (near-miss, out-of-scope, floor). ADR 0005 records why probabilistic behaviour cannot be unit-tested: the hermetic suite proves wiring, the golden set is the manual quality tripwire. Outcome column is graded by hand on the deployed page. **The set then proved itself (2026-08-25):** row 3's live run refused honestly from missing evidence — top-k=5's small RRF pool let keyword overlap crowd out the travel chunk. top_k raised to 10 (ADR 0004 amendment), row re-graded. A unit test could never have caught this. |
| 1.5 | ✅ README — problem statement, mermaid architecture diagram, live URL, screenshot placeholders (login-gated — captured by hand), the three-layer testing story (hermetic suite / CI gates / live golden set), and a concrete what-changes-at-scale section giving every sizing choice an upgrade path. `.env.example` brought up to date with the full config surface. |
| 1.5 | ✅ **[CP]** Full interview rehearsal (2026-08-30/31). Agent interrogated every decision across the week and scored each answer; the weak-areas report (vocabulary precision, multi-part questions, story structure, scale paths) and the three model sentences were handed back for rehearsal. The retest moved the transaction question from 2/5 to 4/5. |
