...
| 1.5 | ✅ README — problem statement, mermaid architecture diagram, live URL, screenshot placeholders (login-gated — captured by hand alongside the recording), the three-layer testing story (hermetic suite / CI gates / live golden set), and a concrete what-changes-at-scale section giving every sizing choice an upgrade path. `.env.example` brought up to date with the full config surface. |
